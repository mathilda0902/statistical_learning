...
def jaccard(n_x, yr, min_support):
    """Compute the Jaccard similarity between all pairs of
    users.

    Only **common** users (or items) are taken into account. The Jaccard similarity is defined as:

    .. math ::
        \\text{jaccard}(u, v) = \\frac{|I_{uv}|}{|I_{u}| + |I_{v}| - |I_{uv}|}
    ...
    # number of common ys
    cdef np.ndarray[np.int_t, ndim=2] frequ
    # number of common ys
    cdef np.ndarray[np.int_t, ndim=2] freqv
    # number of common ys
    cdef np.ndarray[np.int_t, ndim=2] frequv
    # the similarity matrix
    cdef np.ndarray[np.double_t, ndim=2] sim
    ...
    frequ = np.zeros((n_x, n_x), np.int)
    freqv = np.zeros((n_x, n_x), np.int)
    frequv = np.zeros((n_x, n_x), np.int)
